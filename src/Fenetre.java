...
public class Fenetre extends JFrame {
  ...
  public Fenetre() {
    ...
    this.setTitle("Renommeur de séries");
  ...
  private void initMenu()// mettre les éléments dans la barre de menu();
  {
    ...
    menuBar.add(aide);
    fichier.add(mChoix);
    fichier.add(mDemarrer);
    fichier.add(mQuitter);
    aide.add(mAide);
    aide.add(mAPropos);
  ...
  private void initComponents() // mettre les composants dans la fenetre
  {
    ...
    panels[0].add(labelChoix);
    panels[0].add(textChoix);
    panels[0].add(btChoix);
    panels[1].add(labelSerie);
    panels[1].add(textSerie);
    panels[1].add(labelModele);
    panels[1].add(comboModele);
    panels[2].add(check1);
    panels[2].add(labelTerminaison);
    panels[2].add(comboTerminaison);
    panels[3].add(btDemarrer);
    textChoix.setPreferredSize(new Dimension(363, 27));
  ...
  public class choixListener implements ActionListener {
    public void actionPerformed(ActionEvent arg0) {
      JFileChooser chooser = new JFileChooser(new File(System.getProperty("user.home") + "\\Videos"));
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
        monFichier = chooser.getSelectedFile();
        textChoix.setText(monFichier.getAbsolutePath());
    ...
    public void actionPerformed(ActionEvent arg0) {
      if (monFichier.exists()) {
        if (textSerie.getText().equals("")) {
          JOptionPane.showMessageDialog(null, "Vous devez renseigner le nom de la série!", "Attention",
        ...
        JOptionPane.showMessageDialog(null, "Vous devez choisir un dossier ou un fichier!", "Attention",
            JOptionPane.WARNING_MESSAGE);
      }
  ...
  public int getSaison(String chemin, String modele) {
    int pos = -1;
    String utile = chemin.toLowerCase(), utileFinal = "";
    int b1 = -1, b2 = -1;
    int saison = -1;
    ...
    if (pos != -1) {
      utile = utile.substring(pos);
      // System.out.println(utile);
      try {
        utileFinal = utile.substring(modele.length(), modele.length() + 2);
        // System.out.println(utileFinal + "#" + utileFinal.length());
      } catch (Exception e1) {
        e1.printStackTrace();
      }
      try {
        b1 = Integer.valueOf(String.valueOf(utileFinal.charAt(0)));
        b2 = Integer.valueOf(String.valueOf(utileFinal.charAt(1)));
      } catch (Exception e) {
        /* e.printStackTrace(); */}
      System.out.println(b1 + "   " + b2);
      if (b1 != -1) {
        saison = (b2 == -1) ? b1 : Integer.valueOf(utileFinal);
      } else {
        return getSaison(utile.substring(modele.length()), modele);
      }
    } else {
    }
    return saison;
  ...
  public int getSaison(String chemin) {
    String[] modeles = { "saison ", "s", "saison", "saison.", "season ", "season",
        "season." };
    for (String str : modeles) {
      int i = getSaison(chemin, str);
      if (i > 0)
        return i;
    }
    this.erreurs += "(technique 1) ECHEC D'OBTENTION DE LA SAISON POUR: " + chemin + "\n";
    return -1;
  }

  public int getEpisode(String chemin) {
    String[] modeles = { "épisode ", "episode ", "e", "épisode.", "episode.", "épisode", "episode", "episod ", "episod",
        "episod." };
    for (String str : modeles) {
      int i = getSaison(chemin, str);
      if (i > 0)
    ...
    this.erreurs += "(technique 1) ECHEC D'OBTENTION DE L'EPISODE POUR: " + chemin + "\n";
    return -1;
  }

  public int[] getSaisonEpisode(String chemin) {
    System.out.println("Technique 2");

    Matcher m = Pattern.compile("(\\d+)[\\s)\u2013_.-]+(\\d+)").matcher(chemin);

    if (m.find()) {
      int[] result = new int[2];
      result[0] = Integer.valueOf(m.group(1)); // saison
      result[1] = Integer.valueOf(m.group(2)); // épisode
  ...
  public String getNomFinal(String chemin, int mode, String serie, boolean replaceSpaceByPoint, String terminaison) {
    int saison = getSaison(chemin), episode = getEpisode(chemin);
    String nomFinal = "";

    if (!(saison > 0 && episode > 0)) { // Si la technique 1 n'a pas marché (on n'a pas pu obtenir la saison ou
                                        // l'épisode) alors ...
      // Technique 2
      int[] result = getSaisonEpisode(chemin);
      if (result != null) {
        saison = result[0];
        episode = result[1];
      }
    ...
    if (saison > 0 && episode > 0) {
      ...
      if (mode == 1) {
        nomFinal = serie + " S" + saison2 + "E" + episode2;
      } else if (mode == 2) {
        nomFinal = serie + " S" + saison + "E" + episode;
      } else if (mode == 3) {
        nomFinal = serie + " saison " + saison + " épisode " + episode;
      }

      if (!terminaison.equals(""))
        nomFinal += (" " + terminaison);
      if (replaceSpaceByPoint)
        nomFinal = nomFinal.replace(" ", ".");
      return nomFinal;
    }
  ...
  public void renameSerie(File f) // f peut être un fichier ou un dossier
  {
    if (f.exists()) {
      if (!f.isDirectory()) {
    	  ...
    	  if(! Pattern.compile("\\.(avi|flv|mkv|mov|mp4|m4b|ts)", Pattern.CASE_INSENSITIVE).matcher(extension).matches()) return;
        ...
        if (f.getName().equals(nomFinal))
          ...
          f.renameTo(new File(f.getParentFile() + File.separator +
              nomFinal + extension));
      }
      ...
      System.out.println("Le fichier/dossier n'existe pas");
